...
/**
 * Shared by the dev seeder (test@example.com) and the on-registration
 * listener, so every account — seeded or signed up for real — starts
 * with a working example instead of an empty screen list.
 */
class CreatesDemoScreen
{
    ...
    public function for(User $user): Screen
    {
        // Via the relation, not Screen::firstOrCreate(['user_id' => ...]) — user_id
        // isn't fillable, so mass assignment would silently drop it.
        $screen = $user->screens()->firstOrCreate(
            ['name' => 'Demo Board'],
            ['sort_order' => 0],
        );
